...
def can_assign(drone, delivery, current_time):
    ...
    if not (start <= current_time and (current_time + delivery_duration) <= end):
        return False
...
def generate_smart_individual(drones, deliveries, start_time="08:00"):
    individual = {f"D{d.id}": [] for d in drones}
    remaining_capacity = {f"D{d.id}": d.max_weight for d in drones}
    drone_time = {f"D{d.id}": datetime.strptime(start_time, "%H:%M") for d in drones}  # tous les drones commencent à 08:00
    ...
    for delivery in sorted_deliveries:
        ...
        for drone in drones:
            key = f"D{drone.id}"
            if can_assign(drone, delivery, drone_time[key]) and remaining_capacity[key] >= delivery.weight:
                candidate_drones.append(key)

        if candidate_drones:
            chosen_key = random.choice(candidate_drones)
            #let's taqe the drone
