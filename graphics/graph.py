...
def plot_combined_graph_and_path(deliveries, graph, no_fly_zones, path):
    ...
    # Plot the graph edges (connections)
    ...
    if path:
        ...
        ax.plot(x_coords, y_coords, marker='o', color='blue', label='Path', linestyle='-', markersize=8)
        ...
        for i in range(len(path) - 1):
            ...
            ax.arrow(x_start, y_start, dx * 0.8, dy * 0.8,  # Scale arrow size
...
def plot_combined_oriented_graph_and_path(deliveries, graph, no_fly_zones, path):
    # Create a single figure and axis for both the graph and the path
    fig, ax = plt.subplots(figsize=(10, 10))
...
def plot_oriented_graph(deliveries, graph, no_fly_zones):
    fig, ax = plt.subplots()
